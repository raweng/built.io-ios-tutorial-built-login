...
Used as listerner when presence is changed
     ...
     //Obj-C
     [userObject onPresenceChange:^{
         //presence changed
     }];
     
     //Swift
     userObject.onPresenceChange { () -> Void in
         //presence changed
     }
 ...
 */
- (void)onPresenceChange:(void (^) (void))callbackBlock;
...
Request for user's presence
     ...
     [userObject requestForPresenceOnCompletion:^(NSError *error) {
         if (error) {
     ...
     userObject.requestForPresenceOnCompletion { (error) -> Void in
         if (error != nil) {
            //error occured while requesting for presence
         }else {
            //requested for presence
 ...
 */
- (void)requestForPresenceOnCompletion:(void (^) (NSError *error))completionBlock;
