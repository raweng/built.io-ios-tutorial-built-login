...
/**
Include custom option in key value string
 
     //Obj-C
     BuiltApplication *builtApplication = [Built applicationWithAPIKey:@"blt5d4sample2633b"];
     BuiltClass *projectClass = [builtApplication classWithUID:@"project"];
     BuiltQuery *projectQuery = [projectClass query];
     [projectQuery includeKey:@"query_key" andValue:@"query_value"];
 ...
 @discussion Include custom option in key value string.
 @param key Query name to include.
 @param value Query value to include.
 */
- (void)includeKey:(NSString *)key andValue:(NSString *)value;
...
/**
 @abstract Remove provided query key from custom query if exist.
     ...
     BuiltClass *projectClass = [builtApplication classWithUID:@"project"];
     BuiltQuery *projectQuery = [projectClass query];
     [projectQuery removeKey:@"query_key"];
...
/**
 @abstract Returns objects before specified UID.
     ...
     [projectQuery beforeUID:@"blt0ae4df463e93f6f5"];
...
/**
 @abstract Returns objects after specified UID.
     ...
     BuiltQuery *projectQuery = [projectClass query];
     // 'blt0ae4df463e93f6f5' is dummy uid
     [projectQuery afterUID:@"blt0ae4df463e93f6f5"];
...
/**
 @abstract Include reference objects with given key in response
     ...
     [commentQuery includeRefFieldWithKey:@[@"for_bug"]];
...
/**
 @abstract Specifies an array of 'only' keys in reference class object that would be included in the response.
     ...
     BuiltClass *commentClass = [builtApplication classWithUID:@"comment"];
     BuiltQuery *commentQuery = [commentClass query];
     [commentQuery includeRefFieldWithKey:@[@"for_bug"] andOnlyRefValuesWithKeys:@[@"name",@"description"]];
...
/**
 @abstract Specifies an array of keys in reference class object that would be 'excluded' from the response.
     ...
     BuiltClass *commentClass = [builtApplication classWithUID:@"comment"];
     BuiltQuery *commentQuery = [commentClass query];
     [commentQuery includeRefFieldWithKey:@[@"for_bug"] excludingRefValuesWithKeys:@[@"description"]];
...
/**
 @abstract Executes a Query synchronously.
 ...
 @return Returns BuiltSynchronousResponse wrapper comprising response type (cache or network) and response data holding QueryResult object.
 */
- (BuiltSynchronousResponse *)exec:(NSError **)error;
 ...
 */
- (void)execInBackground:(void (^) (ResponseType type, QueryResult *result, NSError *error))completionBlock;
