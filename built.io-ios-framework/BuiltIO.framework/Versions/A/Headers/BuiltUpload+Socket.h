...
For list of supported events, check [EventType](/Constants/EventType.html)
     ...
     [uploadObject on:EventTypeCreate callback:^(id response, NSError *error) {
         if (error) {
     ...
     uploadObject.on(EventType.Create, callback: { (response, error) -> Void in
         if (error != nil) {
            //some error occured while listening
         }else {
            //BuiltUpload instance in response which is recently created
...
For list of supported events, check [EventType](/Constants/EventType.html)
     ...
     //just for example we are using EventTypeCreate.
     [uploadObject off:EventTypeCreate callback:^(id response, NSError *error) {
         if (error) {
     ...
     //just for example we are using EventType.Create.
     uploadObject.off(EventType.Create, callback: { (response, error) -> Void in
         if (error != nil) {
            //some error occured while unregister
         }
     })
